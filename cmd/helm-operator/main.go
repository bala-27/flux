...
func main() {
	...
	rel := release.New(log.With(logger, "component", "release"), helmClient, checkoutFhr)
	relsync := releasesync.New(log.With(logger, "component", "releasesync"), rel)

	// CHARTS CHANGES SYNC -----------------------------------------------------------------------------
	chartSync := chartsync.New(log.With(logger, "component", "chartsync"), *chartsSyncInterval, *chartsSyncTimeout, *kubeClient, *ifClient, rel, *relsync)
